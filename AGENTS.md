...
- **Logo walls** (`index.html`): Primary strip **Trusted By**. **Meetings booked at** uses a **marquee** (~32s loop; `prefers-reduced-motion` disables animation and duplicates). Monochrome **raster/SVG** marks use `mark-mono` + **`opt-w-*`** scale steps for even visual weight. **Alignops** uses `mark-on-light`. **Rocket Lawyer** (white-filled SVG) uses **`mark--from-white`**. **Disney** and **McDonald’s** in the meetings strip are **inline text** (`.mark-embed`) because SVG text inside an `img` is unreliable in browsers. Case study header Route: `client-logo--case`. Assets under `assets/logos/`, `assets/logos/recent-2026/`, `assets/logos/clients/`.
